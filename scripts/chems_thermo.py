...
def __generate_conformers(mol, num_confs=20):
    mol = Chem.AddHs(mol)
...
def __conf_to_xyz(mol, conf=None):
    if not conf:
...
def __get_conformers_xyz(mol):
    confs_xyz = []
    for conf in mol.GetConformers():
        xyz_str = __conf_to_xyz(mol, conf)
        confs_xyz.append(xyz_str)
    
    return confs_xyz


def convert_to_xyz_rdkit(input_file, output_dir):
    ...
    for i, entry in enumerate(entries):
        ...
        try:
            ...
            mol = __generate_conformers(mol)
            confs_xyz = __get_conformers_xyz(mol)
...
def __optimize_structure_crest(crest_cmd, file_xyz, work_dir, chem, result_path):
    ...
    for try_i in range(perturb_tries_num):
        ...
        xyz_str = __conf_to_xyz(perturbed_mol)
        file_xyz.seek(0)
        file_xyz.truncate()
...
def generate_missing_structures(conformers_dir, chems_jsonl, crest_cmd='crest', obabel_cmd='obabel'):
    ...
    for chem in chems:
        try:
            ...
            with NamedTemporaryFile(suffix='.xyz', delete=True) as tmp, TemporaryDirectory(suffix='_tmp') as work_dir:
                ...
                status = __optimize_structure_crest(crest_cmd, tmp, work_dir, chem, result_path)
                if not status:
            ...
            mol_opt = __generate_conformers(mol_opt)
            confs_xyz = __get_conformers_xyz(mol_opt)
            if(len(confs_xyz) == 0):
...
def __compute_boltzmann_average(results):
...
def __run_xtb_single(conformers_xyz, max_conformers=5, xtb_cmd='xtb', method='gfn2'):
    results = []
...
def run_xtb(conformers_dir, output_file, order_file=None, xtb_cmd='xtb'):
    ...
    with open(output_file, 'a') as f_out:
        ...
        for fn in order if order_file else filenames:
            ...
            res = __run_xtb_single(conforms_xyz, max_conformers=max_conformers, xtb_cmd=xtb_cmd)
            if res:
...
def clear_null_enthalpy_entries(filename):
